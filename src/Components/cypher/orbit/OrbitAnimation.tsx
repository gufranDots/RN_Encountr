...
type Props = {
  ...
  idSuffix?: string;
  /** When true, renders the full-screen radial backdrop (standalone splash). */
  showBackground?: boolean;
  style?: ViewStyle;
};

// Orb-only crop: square viewBox centered on the orb (no tall letterbox).
const ORB_VB = { x: CX - 205, y: CY - 205, w: 410, h: 410 };
...
export default function OrbitAnimation({
  ...
  const svgHeight = showBackground ? (dim * VB_H) / VB_W : dim;
  const viewBox = showBackground
    ? `0 0 ${VB_W} ${VB_H}`
    : `${ORB_VB.x} ${ORB_VB.y} ${ORB_VB.w} ${ORB_VB.h}`;
  ...
  return (
    <View style={[{ width: dim, height: svgHeight, backgroundColor: 'transparent' }, style]}>
      <Svg width={dim} height={svgHeight} viewBox={viewBox}>
        <Defs>
          {showBackground && (
        ...
        <AnimatedG animatedProps={burstProps} opacity={0.55}>
          <Circle cx={CX} cy={CY} r={40} fill={`url(#burst-${s})`} opacity={0.75} />
          {/* 4 long cardinal rays */}
          <Polygon points={`${CX},${CY - 92} ${CX + 8},${CY - 8} ${CX},${CY} ${CX - 8},${CY - 8}`} fill="#ffffff" opacity={0.7} />
          <Polygon points={`${CX},${CY + 92} ${CX + 8},${CY + 8} ${CX},${CY} ${CX - 8},${CY + 8}`} fill="#ffffff" opacity={0.7} />
          <Polygon points={`${CX - 92},${CY} ${CX - 8},${CY + 8} ${CX},${CY} ${CX - 8},${CY - 8}`} fill="#ffffff" opacity={0.7} />
          <Polygon points={`${CX + 92},${CY} ${CX + 8},${CY + 8} ${CX},${CY} ${CX + 8},${CY - 8}`} fill="#ffffff" opacity={0.7} />
          {/* 4 short diagonal rays */}
          <G transform={`rotate(45 ${CX} ${CY})`} opacity={0.5}>
          ...
          <Circle cx={CX} cy={CY} r={19} fill={P.coreA} opacity={0.35} />
        </AnimatedG>
