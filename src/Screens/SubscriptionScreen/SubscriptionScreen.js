...
const SubscriptionScreen = ({ navigation, route }) => {
  ...
  console.log(connected, 'is connected -->>>>>>>>>>>', subscriptions)

  useEffect(() => {
    const basicPlan = []
    const blueSubscription = []
    const pinkSubscription = []
    const crystalSubscription = []
    if (subscriptions) {
      subscriptions.map(val => {
        if (val?.productId?.includes('subscription.blue')) {
          blueSubscription.push(val)
        }
        if (val?.productId?.includes('subscription.pink')) {
          pinkSubscription.push(val)
        }
        if (val?.productId?.includes('subscription.crystal')) {
          crystalSubscription.push(val)
        }
        return null
      })
      setSubscriptionData([
  ...
  const handleBuySubscription = async (productId, offerToken) => {
    ...
    console.log(productId, 'roduct is ==>>')
    if (!offerToken) {
      console.log(
  ...
  console.log(subscriptionData, 'sl;ks;lksal;k;l')
    ...
    return (
      <ImageBackground
        ...
        <View
          ...
          <Text style={{ ...commonStyles.font_16_bold, color: theme.colors.black }}>
            {item?.name
              ? item?.name
              : index === 0
                ? 'Basic'
    ...
    <ImageBackground source={imagesPath.subscription} style={{ flex: 1 }}>
      ...
      <BottomSheet
        ...
        <View style={styles.bottomSheetView}>
          ...
          {console.log(activePlan, 'l;kasd;lsak;alsk;')}
          <FlatList
            data={activePlan && activePlan?.subscriptionOfferDetails}
            numColumns={3}
            renderItem={Platform.OS === 'ios' ? renderItemIOS : renderItem}
          ...
          <ButtonComp
            ...
            onPressBtn={() =>
              handleBuySubscription(
                activePlan?.subscriptionOfferDetails[selectedIndex || 0]?.productId,
                activePlan &&
                  activePlan?.subscriptionOfferDetails[selectedIndex]
                    ?.offerToken
              )
