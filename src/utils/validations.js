...
export const checkPasswordValidations = (password, message) => {
  if (checkIsEmpty(password)) {
    return showError(`Please enter a valid ${message} password.`);
...
export const BonkersFilterValidations = (
  ...
  if (!bodyTypeValue?.name) {
    showError('Please choose your preferred body type');
    return false;
  }
  if (!lookingFor?.name) {
    showError('Please choose your looking for preferrence');
    return false;
  }
  // if (!ink?.name) {
  ...
  if (!maritalStatusValue?.name) {
    showError('Please choose your preferred relationship status');
    return false;
  }
  if (!isChildrenValue?.name) {
    showError('Please choose your preferred kids option');
    return false;
  }
  // if (!lifestyle?.name) {
